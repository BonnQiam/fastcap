...
#ifdef WIN32
        {
            // We need the file opened in binary mode in order for
            // fseek/ftell to work properly in MinGW.

            FILE *fp1 = fopen(list_file, "rb");
            if (!fp1) {
                fprintf(stderr,
                    "read_list_file: can't open list file\n  `%s'\nto read\n", 
                    list_file);
                exit(1);
            }
            setup_file_offsets(fp1);
            fclose(fp1);
        }
#else
